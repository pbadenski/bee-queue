...
class Job extends Emitter {
  ...
  save(cb) {
    ...
    let promise = this.queue._evalScript('createJobId', 1, toKey('id'), this.id || '');
    ...
    if (this.options.delay) {
      promise = promise.then(() => this.queue._evalScript('addDelayedJob', 4,
      ...
      promise = promise.then(() => this.queue._evalScript('addJob', 3,
